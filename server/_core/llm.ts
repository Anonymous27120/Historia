...
// Works with OpenAI's API out of the box, or any OpenAI-compatible gateway
// (set OPENAI_BASE_URL to point elsewhere).
const resolveApiUrl = () =>
...
const assertApiKey = () => {
  if (!ENV.openaiApiKey) {
    throw new Error(
      "OPENAI_API_KEY is not configured. Set it in your hosting provider's environment variables."
    );
  }
};
